    ...
    def __init__(self, classes=13, fuse_stages=4, planes=16, output=196):
        ...
        for index in range(self.FMF_stages + 1):
            if index == 0:
                ...
                baseline_relu.append(nn.ReLU())
                baseline_res_1.append(
                    Bottleneck(self.planes * pow(2, index), self.planes * pow(2, index - 1), stride=2))
        ...
        for index in range(self.FMF_stages):
            ...
            pre_fmf_conv.append(
                nn.Conv2d(self.planes * pow(2, index + 1) + self.classes, self.planes * pow(2, index + 1),
...
def load_align_weights(net, pth_file, device):
...
class Get_alignment(nn.Module):

    def __init__(self, path_estimator, path_regressor):
        super(Get_alignment, self).__init__()

        self.estimator = Estimator()
        self.regressor = Regressor(output=2 * 98)
        self.estimator = load_align_weights(self.estimator, path_estimator, device)
        self.regressor = load_align_weights(self.regressor, path_regressor, device)
        if use_cuda:
            self.estimator = self.estimator.cuda(device=device)
            self.regressor = self.regressor.cuda(device=device)
        self.estimator.eval()
        self.regressor.eval()
...
def warp_im(tar_landmarks, gl_landmarks):
    pts1 = np.float64(np.matrix([[point[0], point[1]] for point in tar_landmarks]))
    # print(gl_landmarks)

    pts2 = np.float64(np.matrix([[point[0], point[1]] for point in gl_landmarks]))
